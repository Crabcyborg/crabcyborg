	...
	view: v => [
		...
		m(
			...
			m(
				...
				{
					i: 'shapeup',
					configuration: () => shapes[configuration_keys[configuration_index]],
					size: shape_size,
			...
			m(
				'.absolute',
				{
					style: {
						top: 0,
						bottom: 0,
						left: (shapes[configuration_keys[configuration_index]][1]*shape_size + 60)+'px',
						width: (shapes[configuration_keys[configuration_index]][1]*target_size) * 1.25+'px',
						border: '1px solid #efefef'
					}
				},
				m(TargetShape, {i: 'shapeup', size: target_size, style: { top: '50%', left: '50%', transform: 'translateX(-50%) translateY(-50%)' }})
			),
			m(Score, {i: 'shapeup', style: { position: 'absolute', left: (shapes[configuration_keys[configuration_index]][1]*(shape_size+target_size*1.25) + 80)+'px' }}),
			m(
				Trigger,
				{
					onclick: () => {

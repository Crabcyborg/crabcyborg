...
let width, height, ready = false, rects, larger_rects, square_polygons, polygons, active_color;
...
const toPolygons = cells => {
	let points_by_color_index = groupPointsByColorIndex(cells, size), output = [];
		...
		[
			...
			ready && m(
				...
				m(
					...
					rects.map(
						cell => (active_color === undefined || active_color === cell.fill)
						&& m(
							'rect',
							{
								...cell,
								stroke: 'white',
								onclick: () => {
									if(active_color === undefined || active_color !== cell.fill) {
										active_color = cell.fill;
									} else {
				...
				m(
					Container,
					{ data: larger_rects, label: 'larger rects'},
					larger_rects.map(cell => m('rect', cell))
				),
				m(
					Container,
			...
			m(ShapeUp, {configuration: shapes.LUNAR, size, /*behaviour: 'blink', blink_delay: 1000,*/ onUpdate: shapeup => {
				...
				larger_rects = toLargerRects(filtered);
